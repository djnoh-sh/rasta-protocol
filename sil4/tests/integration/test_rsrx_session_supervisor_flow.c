...
static void vTestIntegratedHoldoffStaleCompletionRecoveryOrderingFlow(void)
{
	...
	vAssertTrue(pxSupervisorReport->eLastDecision == RSRX_SUPERVISOR_DECISION_SEND_FEEDBACK_UNCORRELATED_IGNORED, "holdoff stale completion recovery ordering integration stale secondary decision");
	vAssertTrue(rsrx_transport_adapter_has_outstanding_send(&xSession.xTransportAdapter) == 1U, "holdoff stale completion recovery ordering integration stale secondary outstanding retained");
