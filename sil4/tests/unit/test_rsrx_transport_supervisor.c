...
static void vTestSupervisorRastaSrRuntimeTimestampAdmission(void)
{
	rsrx_session_t xSession;
	rsrx_session_config_t xConfig;
	rsrx_transport_supervisor_context_t xSupervisor;
	...
	static const uint8_t auPayload[2] = { 0xA5U, 0x5AU };

	vInitTransportContext(&xTransport, auPayload, sizeof(auPayload), RSRX_TRANSPORT_EVENT_FRAME_RECEIVED);
	vFillConfig(&xConfig, &xTransport, &xClock, &xTimer, &xDiagnostics, &xCallbacks, auPayload, sizeof(auPayload));
	vAssertTrue(rsrx_session_init(&xSession, &xConfig) == RSRX_STATUS_OK, "rasta sr runtime session init");
	vAssertTrue(rsrx_session_start(&xSession, &pxSessionReport) == RSRX_STATUS_OK, "rasta sr runtime session start");
	vAssertTrue(rsrx_session_connect(&xSession, &pxSessionReport) == RSRX_STATUS_OK, "rasta sr runtime session connect");
	vAssertTrue(rsrx_session_process_event(&xSession, RSRX_EVENT_HANDSHAKE_SUCCESS, &pxSessionReport) == RSRX_STATUS_OK, "rasta sr runtime establish");
	...
	vSetCodecBehavior(
		...
		0U,
		0U);
	vAssertTrue(rsrx_transport_supervisor_init(&xSupervisor, &xSession, &xCodec) == RSRX_SUPERVISOR_STATUS_OK, "rasta sr runtime supervisor init");
	...
	xRequest.uReceiverId = 0x1000U;
	xRequest.uSenderId = 0x2000U;
	xRequest.uSequenceNumber = 1U;
	xRequest.uConfirmedSequenceNumber = 0U;
	xRequest.uTimestamp = 1001U;
	xRequest.uConfirmedTimestamp = 1000U;
	...
	vAssertTrue(
		...
		"rasta sr runtime encode accepted frame");
	xFrame.eChannelId = RSRX_TRANSPORT_CHANNEL_PRIMARY;
	...
	vAssertTrue(
		rsrx_transport_supervisor_process_frame(&xSupervisor, &xFrame, &pxSupervisorReport) ==
			RSRX_SUPERVISOR_STATUS_OK,
		"rasta sr runtime accepted frame");
	vAssertTrue(g_xCodecContext.uCallCount == 0U, "rasta sr runtime bypasses legacy codec port");
	vAssertTrue(pxSupervisorReport->xLastMessage.eMessageType == RSRX_MESSAGE_TYPE_DATA, "rasta sr runtime message type");
	vAssertTrue(pxSupervisorReport->xLastMessage.xPayloadLength == sizeof(auPayload), "rasta sr runtime payload length");
	...
	xRequest.uSequenceNumber = 2U;
	xRequest.uTimestamp = 1100U;
	xRequest.uConfirmedTimestamp = 1000U;
	xBuffer.xEncodedLength = 0U;
	vAssertTrue(
		rsrx_codec_encode_rasta_sr_no_checksum(&xRequest, &xBuffer) == RSRX_CODEC_STATUS_OK,
		"rasta sr runtime encode future frame");
	xFrame.xPayloadLength = xBuffer.xEncodedLength;
	vAssertTrue(
		rsrx_transport_supervisor_process_frame(&xSupervisor, &xFrame, &pxSupervisorReport) ==
			RSRX_SUPERVISOR_STATUS_DECODE_FAILED,
		"rasta sr runtime rejects future timestamp");
	vAssertTrue(pxSupervisorReport->eLastCodecStatus == RSRX_CODEC_STATUS_TIMESTAMP_IN_FUTURE, "rasta sr runtime future status");
...
static void vTestSupervisorSwitchAuditTerminalOutcomeThresholdNineteenMixedLongRunMatrix(void);
static void vTestSupervisorSwitchAuditTerminalOutcomeThresholdTwentyMixedLongRunMatrix(void);
...
static void vTestSupervisorSwitchAuditTerminalOutcomeEnvelopeMatrix(void)
{
	...
	vTestSupervisorSwitchAuditTerminalOutcomeThresholdNineteenMixedLongRunMatrix();
	vTestSupervisorSwitchAuditTerminalOutcomeThresholdTwentyMixedLongRunMatrix();
}
...
static void vTestSupervisorSwitchAuditTerminalOutcomeThresholdNineteenMixedLongRunMatrix(void)
{
	...
	xTransport.uSecondaryAvailable = 1U;
	vFillConfig(&xConfig, &xTransport, &xClock, &xTimer, &xDiagnostics, &xCallbacks, auPayload, sizeof(auPayload));
	vSetActiveStandbyHoldoffConfig(&xConfig, 19U);

	vAssertTrue(rsrx_session_init(&xSession, &xConfig) == RSRX_STATUS_OK, "switch audit terminal outcome threshold nineteen mixed long-run matrix session init");
	vAssertTrue(rsrx_session_start(&xSession, &pxSessionReport) == RSRX_STATUS_OK, "switch audit terminal outcome threshold nineteen mixed long-run matrix session start");
	vAssertTrue(rsrx_session_connect(&xSession, &pxSessionReport) == RSRX_STATUS_OK, "switch audit terminal outcome threshold nineteen mixed long-run matrix session connect");
	vAssertTrue(rsrx_session_process_event(&xSession, RSRX_EVENT_HANDSHAKE_SUCCESS, &pxSessionReport) == RSRX_STATUS_OK, "switch audit terminal outcome threshold nineteen mixed long-run matrix establish");

	vAssertTrue(rsrx_transport_supervisor_init(&xSupervisor, &xSession, &xCodec) == RSRX_SUPERVISOR_STATUS_OK, "switch audit terminal outcome threshold nineteen mixed long-run matrix supervisor init");
	...
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_DOWN;
	xTransport.uPrimaryAvailable = 0U;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold nineteen mixed long-run matrix first failover");

	xTransport.uPrimaryAvailable = 1U;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_UP;
	for(uHoldIndex = 1U; uHoldIndex < 19U; ++uHoldIndex)
	{
		vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold nineteen mixed long-run matrix hold");
		vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffProgressCount == uHoldIndex, "switch audit terminal outcome threshold nineteen mixed long-run matrix hold progress");
		vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffRemainingCount == (19U - uHoldIndex), "switch audit terminal outcome threshold nineteen mixed long-run matrix hold remaining");
		vAssertTrue(pxSupervisorReport->uTerminalHoldoffOutcomeCount == 0U, "switch audit terminal outcome threshold nineteen mixed long-run matrix hold terminal count");
	}
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold nineteen mixed long-run matrix ordinary completion");
	vAssertTrue(pxSupervisorReport->uTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold nineteen mixed long-run matrix ordinary total count");
	vAssertTrue(pxSupervisorReport->uOrdinaryTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold nineteen mixed long-run matrix ordinary subtype count");
	vAssertTrue(pxSupervisorReport->uBypassTerminalHoldoffOutcomeCount == 0U, "switch audit terminal outcome threshold nineteen mixed long-run matrix ordinary bypass subtype count");
	vAssertTrue(pxSupervisorReport->uAbortedTerminalHoldoffOutcomeCount == 0U, "switch audit terminal outcome threshold nineteen mixed long-run matrix ordinary aborted subtype count");

	/* cppcheck-suppress redundantAssignment */
	xTransport.uPrimaryAvailable = 0U;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_DOWN;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold nineteen mixed long-run matrix second failover");

	xTransport.uPrimaryAvailable = 1U;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_UP;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold nineteen mixed long-run matrix abort hold");
	vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffProgressCount == 1U, "switch audit terminal outcome threshold nineteen mixed long-run matrix abort hold progress");
	vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffRemainingCount == 18U, "switch audit terminal outcome threshold nineteen mixed long-run matrix abort hold remaining");

	/* cppcheck-suppress redundantAssignment */
	xTransport.uPrimaryAvailable = 0U;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_DOWN;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold nineteen mixed long-run matrix abort reset");
	vAssertTrue(pxSupervisorReport->uTerminalHoldoffOutcomeCount == 2U, "switch audit terminal outcome threshold nineteen mixed long-run matrix abort total count");
	vAssertTrue(pxSupervisorReport->uOrdinaryTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold nineteen mixed long-run matrix abort ordinary subtype count");
	vAssertTrue(pxSupervisorReport->uBypassTerminalHoldoffOutcomeCount == 0U, "switch audit terminal outcome threshold nineteen mixed long-run matrix abort bypass subtype count");
	vAssertTrue(pxSupervisorReport->uAbortedTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold nineteen mixed long-run matrix abort aborted subtype count");

	xTransport.uPrimaryAvailable = 1U;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_UP;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold nineteen mixed long-run matrix renewed hold");
	vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffProgressCount == 1U, "switch audit terminal outcome threshold nineteen mixed long-run matrix renewed hold progress");
	vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffRemainingCount == 18U, "switch audit terminal outcome threshold nineteen mixed long-run matrix renewed hold remaining");

	xTransport.uSecondaryAvailable = 0U;
	xFrame.eChannelId = RSRX_TRANSPORT_CHANNEL_SECONDARY;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_DOWN;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold nineteen mixed long-run matrix bypass completion");
	vAssertTrue(pxSupervisorReport->uTerminalHoldoffOutcomeCount == 3U, "switch audit terminal outcome threshold nineteen mixed long-run matrix final total count");
	vAssertTrue(pxSupervisorReport->uOrdinaryTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold nineteen mixed long-run matrix final ordinary subtype count");
	vAssertTrue(pxSupervisorReport->uBypassTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold nineteen mixed long-run matrix final bypass subtype count");
	vAssertTrue(pxSupervisorReport->uAbortedTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold nineteen mixed long-run matrix final aborted subtype count");
	vAssertTrue(pxSupervisorReport->uPreferredChannelTriggeredTerminalHoldoffOutcomeCount == 2U, "switch audit terminal outcome threshold nineteen mixed long-run matrix final preferred-triggered count");
	vAssertTrue(pxSupervisorReport->uNonPreferredChannelTriggeredTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold nineteen mixed long-run matrix final non-preferred-triggered count");
	vAssertTrue(pxSupervisorReport->uChannelUpTriggeredTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold nineteen mixed long-run matrix final channel-up count");
	vAssertTrue(pxSupervisorReport->uChannelDownTriggeredTerminalHoldoffOutcomeCount == 2U, "switch audit terminal outcome threshold nineteen mixed long-run matrix final channel-down count");
	vAssertTrue(pxSupervisorReport->eLastHoldoffCycleStartTriggerEventType == RSRX_TRANSPORT_EVENT_CHANNEL_UP, "switch audit terminal outcome threshold nineteen mixed long-run matrix final start trigger event");
	vAssertTrue(pxSupervisorReport->eLastHoldoffCycleStartTriggerChannelId == RSRX_TRANSPORT_CHANNEL_PRIMARY, "switch audit terminal outcome threshold nineteen mixed long-run matrix final start trigger channel");
	vAssertTrue(pxSupervisorReport->eLastTerminalHoldoffOutcome == RSRX_SUPERVISOR_TERMINAL_HOLDOFF_OUTCOME_BYPASS_COMPLETED, "switch audit terminal outcome threshold nineteen mixed long-run matrix final terminal outcome");
	vAssertTrue(pxSupervisorReport->eLastCompletedHoldoffCycleKind == RSRX_SUPERVISOR_COMPLETED_HOLDOFF_CYCLE_KIND_BYPASS, "switch audit terminal outcome threshold nineteen mixed long-run matrix final completed kind");
	vAssertTrue(pxSupervisorReport->eLastHoldoffCycleState == RSRX_SUPERVISOR_HOLDOFF_CYCLE_STATE_COMPLETED, "switch audit terminal outcome threshold nineteen mixed long-run matrix final holdoff state");
}
...
static void vTestSupervisorSwitchAuditTerminalOutcomeThresholdTwentyMixedLongRunMatrix(void)
{
	...
	xTransport.uSecondaryAvailable = 1U;
	vFillConfig(&xConfig, &xTransport, &xClock, &xTimer, &xDiagnostics, &xCallbacks, auPayload, sizeof(auPayload));
	vSetActiveStandbyHoldoffConfig(&xConfig, 20U);

	vAssertTrue(rsrx_session_init(&xSession, &xConfig) == RSRX_STATUS_OK, "switch audit terminal outcome threshold twenty mixed long-run matrix session init");
	vAssertTrue(rsrx_session_start(&xSession, &pxSessionReport) == RSRX_STATUS_OK, "switch audit terminal outcome threshold twenty mixed long-run matrix session start");
	vAssertTrue(rsrx_session_connect(&xSession, &pxSessionReport) == RSRX_STATUS_OK, "switch audit terminal outcome threshold twenty mixed long-run matrix session connect");
	vAssertTrue(rsrx_session_process_event(&xSession, RSRX_EVENT_HANDSHAKE_SUCCESS, &pxSessionReport) == RSRX_STATUS_OK, "switch audit terminal outcome threshold twenty mixed long-run matrix establish");

	vAssertTrue(rsrx_transport_supervisor_init(&xSupervisor, &xSession, &xCodec) == RSRX_SUPERVISOR_STATUS_OK, "switch audit terminal outcome threshold twenty mixed long-run matrix supervisor init");
	...
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_DOWN;
	xTransport.uPrimaryAvailable = 0U;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold twenty mixed long-run matrix first failover");

	xTransport.uPrimaryAvailable = 1U;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_UP;
	for(uHoldIndex = 1U; uHoldIndex < 20U; ++uHoldIndex)
	{
		vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold twenty mixed long-run matrix hold");
		vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffProgressCount == uHoldIndex, "switch audit terminal outcome threshold twenty mixed long-run matrix hold progress");
		vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffRemainingCount == (20U - uHoldIndex), "switch audit terminal outcome threshold twenty mixed long-run matrix hold remaining");
		vAssertTrue(pxSupervisorReport->uTerminalHoldoffOutcomeCount == 0U, "switch audit terminal outcome threshold twenty mixed long-run matrix hold terminal count");
	}
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold twenty mixed long-run matrix ordinary completion");
	vAssertTrue(pxSupervisorReport->uTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold twenty mixed long-run matrix ordinary total count");
	vAssertTrue(pxSupervisorReport->uOrdinaryTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold twenty mixed long-run matrix ordinary subtype count");
	vAssertTrue(pxSupervisorReport->uBypassTerminalHoldoffOutcomeCount == 0U, "switch audit terminal outcome threshold twenty mixed long-run matrix ordinary bypass subtype count");
	vAssertTrue(pxSupervisorReport->uAbortedTerminalHoldoffOutcomeCount == 0U, "switch audit terminal outcome threshold twenty mixed long-run matrix ordinary aborted subtype count");

	/* cppcheck-suppress redundantAssignment */
	xTransport.uPrimaryAvailable = 0U;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_DOWN;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold twenty mixed long-run matrix second failover");

	xTransport.uPrimaryAvailable = 1U;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_UP;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold twenty mixed long-run matrix abort hold");
	vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffProgressCount == 1U, "switch audit terminal outcome threshold twenty mixed long-run matrix abort hold progress");
	vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffRemainingCount == 19U, "switch audit terminal outcome threshold twenty mixed long-run matrix abort hold remaining");

	/* cppcheck-suppress redundantAssignment */
	xTransport.uPrimaryAvailable = 0U;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_DOWN;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold twenty mixed long-run matrix abort reset");
	vAssertTrue(pxSupervisorReport->uTerminalHoldoffOutcomeCount == 2U, "switch audit terminal outcome threshold twenty mixed long-run matrix abort total count");
	vAssertTrue(pxSupervisorReport->uOrdinaryTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold twenty mixed long-run matrix abort ordinary subtype count");
	vAssertTrue(pxSupervisorReport->uBypassTerminalHoldoffOutcomeCount == 0U, "switch audit terminal outcome threshold twenty mixed long-run matrix abort bypass subtype count");
	vAssertTrue(pxSupervisorReport->uAbortedTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold twenty mixed long-run matrix abort aborted subtype count");

	xTransport.uPrimaryAvailable = 1U;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_UP;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold twenty mixed long-run matrix renewed hold");
	vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffProgressCount == 1U, "switch audit terminal outcome threshold twenty mixed long-run matrix renewed hold progress");
	vAssertTrue(pxSupervisorReport->uPreferredRecoveryHoldoffRemainingCount == 19U, "switch audit terminal outcome threshold twenty mixed long-run matrix renewed hold remaining");

	xTransport.uSecondaryAvailable = 0U;
	xFrame.eChannelId = RSRX_TRANSPORT_CHANNEL_SECONDARY;
	xFrame.eEventType = RSRX_TRANSPORT_EVENT_CHANNEL_DOWN;
	vAssertTrue(rsrx_transport_supervisor_process_transport_event(&xSupervisor, &xFrame, &pxSupervisorReport) == RSRX_SUPERVISOR_STATUS_IGNORED_EVENT, "switch audit terminal outcome threshold twenty mixed long-run matrix bypass completion");
	vAssertTrue(pxSupervisorReport->uTerminalHoldoffOutcomeCount == 3U, "switch audit terminal outcome threshold twenty mixed long-run matrix final total count");
	vAssertTrue(pxSupervisorReport->uOrdinaryTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold twenty mixed long-run matrix final ordinary subtype count");
	vAssertTrue(pxSupervisorReport->uBypassTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold twenty mixed long-run matrix final bypass subtype count");
	vAssertTrue(pxSupervisorReport->uAbortedTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold twenty mixed long-run matrix final aborted subtype count");
	vAssertTrue(pxSupervisorReport->uPreferredChannelTriggeredTerminalHoldoffOutcomeCount == 2U, "switch audit terminal outcome threshold twenty mixed long-run matrix final preferred-triggered count");
	vAssertTrue(pxSupervisorReport->uNonPreferredChannelTriggeredTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold twenty mixed long-run matrix final non-preferred-triggered count");
	vAssertTrue(pxSupervisorReport->uChannelUpTriggeredTerminalHoldoffOutcomeCount == 1U, "switch audit terminal outcome threshold twenty mixed long-run matrix final channel-up count");
	vAssertTrue(pxSupervisorReport->uChannelDownTriggeredTerminalHoldoffOutcomeCount == 2U, "switch audit terminal outcome threshold twenty mixed long-run matrix final channel-down count");
	vAssertTrue(pxSupervisorReport->eLastHoldoffCycleStartTriggerEventType == RSRX_TRANSPORT_EVENT_CHANNEL_UP, "switch audit terminal outcome threshold twenty mixed long-run matrix final start trigger event");
	vAssertTrue(pxSupervisorReport->eLastHoldoffCycleStartTriggerChannelId == RSRX_TRANSPORT_CHANNEL_PRIMARY, "switch audit terminal outcome threshold twenty mixed long-run matrix final start trigger channel");
	vAssertTrue(pxSupervisorReport->eLastTerminalHoldoffOutcome == RSRX_SUPERVISOR_TERMINAL_HOLDOFF_OUTCOME_BYPASS_COMPLETED, "switch audit terminal outcome threshold twenty mixed long-run matrix final terminal outcome");
	vAssertTrue(pxSupervisorReport->eLastCompletedHoldoffCycleKind == RSRX_SUPERVISOR_COMPLETED_HOLDOFF_CYCLE_KIND_BYPASS, "switch audit terminal outcome threshold twenty mixed long-run matrix final completed kind");
	vAssertTrue(pxSupervisorReport->eLastHoldoffCycleState == RSRX_SUPERVISOR_HOLDOFF_CYCLE_STATE_COMPLETED, "switch audit terminal outcome threshold twenty mixed long-run matrix final holdoff state");
}
