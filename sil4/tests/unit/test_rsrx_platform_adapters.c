...
static void vTestApplicationDataSend(void)
{
	...
	vAssertTrue(pxTelemetry->uMaxDeferredSendCount == 10U, "application data max deferred telemetry tenth slot");
	vAssertTrue(
		rsrx_transport_adapter_send_application_data(
			&xTransportAdapterContext,
			auDataPayload,
			sizeof(auDataPayload)) == RSRX_TRANSPORT_STATUS_UNAVAILABLE,
		"application data twelfth send overflow");
	...
	vAssertTrue(pxTelemetry->uQueuedSendCount == 11U, "application data queued telemetry after dispatch");
...
static void vTestBusyRejectThresholdManualInboundResetSources(void)
{
	...
	vAssertTrue(
		rsrx_transport_adapter_send_application_data(
			&xTransportAdapterContext,
			auTwelfthPayload,
			sizeof(auTwelfthPayload)) == RSRX_TRANSPORT_STATUS_UNAVAILABLE,
		"busy reject manual inbound reset first reject");
	vAssertTrue(
		rsrx_transport_adapter_send_application_data(
			&xTransportAdapterContext,
			auThirteenthPayload,
			sizeof(auThirteenthPayload)) == RSRX_TRANSPORT_STATUS_UNAVAILABLE,
...
static void vTestOverflowBusyAccumulationMatrix(void)
{
	...
	vAssertTrue(pxTelemetry->uMaxDeferredSendCount == 10U, "overflow busy accumulation max deferred retained");
	...
	vAssertTrue(pxTelemetry->uMaxDeferredSendCount == 10U, "overflow busy accumulation final max deferred retained");
}
