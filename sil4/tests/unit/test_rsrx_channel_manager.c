...
static void vTestPreferredRecoveryFlapPenaltyBypassClear(void)
{
	rsrx_channel_manager_context_t xContext;
	rsrx_channel_manager_config_t xConfig;
	rsrx_channel_selection_result_t xResult;
	...
	vAssertTrue(
		rsrx_channel_manager_init(&xContext, &xConfig) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty bypass init");

	xState.eChannelId = RSRX_TRANSPORT_CHANNEL_PRIMARY;
	xState.uIsAvailable = 0U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty bypass primary down");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty bypass failover");

	xState.eChannelId = RSRX_TRANSPORT_CHANNEL_PRIMARY;
	xState.uIsAvailable = 1U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty bypass primary restored");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty bypass hold");

	xState.uIsAvailable = 0U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty bypass primary flap down");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty bypass arm");
	vAssertTrue(xResult.uPreferredRecoveryPendingPenaltyCount == 1U, "flap penalty bypass pending armed");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyArmCount == 1U, "flap penalty bypass arm count armed");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyClearCount == 0U, "flap penalty bypass ordinary clear count idle");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyBypassClearCount == 0U, "flap penalty bypass bypass clear count idle");

	xState.eChannelId = RSRX_TRANSPORT_CHANNEL_PRIMARY;
	xState.uIsAvailable = 1U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty bypass primary restored again");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty bypass renewed hold");
	vAssertTrue(xResult.uPreferredRecoveryPendingPenaltyCount == 1U, "flap penalty bypass renewed pending");

	xState.eChannelId = RSRX_TRANSPORT_CHANNEL_SECONDARY;
	xState.uIsAvailable = 0U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 1U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty bypass secondary down");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty bypass clear by bypass");
	vAssertTrue(xResult.eSelectedChannelId == RSRX_TRANSPORT_CHANNEL_PRIMARY, "flap penalty bypass recovered primary");
	vAssertTrue(xResult.uPreferredRecoveryPendingPenaltyCount == 0U, "flap penalty bypass pending cleared");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyArmCount == 1U, "flap penalty bypass arm count retained");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyClearCount == 0U, "flap penalty bypass ordinary clear count retained");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyBypassClearCount == 1U, "flap penalty bypass bypass clear count incremented");
}

static void vTestPreferredRecoveryFlapPenaltyAppliedCycleCount(void)
{
	rsrx_channel_manager_context_t xContext;
	rsrx_channel_manager_config_t xConfig;
	rsrx_channel_selection_result_t xResult;
	...
	vAssertTrue(
		rsrx_channel_manager_init(&xContext, &xConfig) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty applied cycle init");

	xState.eChannelId = RSRX_TRANSPORT_CHANNEL_PRIMARY;
	xState.uIsAvailable = 0U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty applied cycle primary down");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty applied cycle failover");

	xState.eChannelId = RSRX_TRANSPORT_CHANNEL_PRIMARY;
	xState.uIsAvailable = 1U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty applied cycle primary restored");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty applied cycle first hold");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyAppliedCycleCount == 0U, "flap penalty applied cycle count idle first hold");

	xState.uIsAvailable = 0U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty applied cycle primary flap down");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty applied cycle arm");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyArmCount == 1U, "flap penalty applied cycle arm count armed");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyAppliedCycleCount == 0U, "flap penalty applied cycle count idle on arm");

	xState.eChannelId = RSRX_TRANSPORT_CHANNEL_PRIMARY;
	xState.uIsAvailable = 1U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty applied cycle primary restored again");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty applied cycle renewed hold one");
	vAssertTrue(xResult.uPreferredRecoveryPendingPenaltyCount == 1U, "flap penalty applied cycle pending retained one");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyAppliedCycleCount == 1U, "flap penalty applied cycle count incremented");

	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty applied cycle renewed hold two");
	vAssertTrue(xResult.uPreferredRecoveryPendingPenaltyCount == 1U, "flap penalty applied cycle pending retained two");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyAppliedCycleCount == 1U, "flap penalty applied cycle count retained");

	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty applied cycle recovery");
	vAssertTrue(xResult.eSelectedChannelId == RSRX_TRANSPORT_CHANNEL_PRIMARY, "flap penalty applied cycle recovered primary");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyClearCount == 1U, "flap penalty applied cycle clear count retained");
...
static void vTestPreferredRecoveryFlapPenaltyResetClearCount(void)
{
	rsrx_channel_manager_context_t xContext;
	rsrx_channel_manager_config_t xConfig;
	rsrx_channel_selection_result_t xResult;
	...
	vAssertTrue(
		rsrx_channel_manager_init(&xContext, &xConfig) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty reset clear init");

	xState.eChannelId = RSRX_TRANSPORT_CHANNEL_PRIMARY;
	xState.uIsAvailable = 0U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty reset clear primary down");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty reset clear failover");

	xState.eChannelId = RSRX_TRANSPORT_CHANNEL_PRIMARY;
	xState.uIsAvailable = 1U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty reset clear primary restored");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty reset clear first hold");

	xState.uIsAvailable = 0U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty reset clear primary flap down");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty reset clear arm");
	vAssertTrue(xResult.uPreferredRecoveryPendingPenaltyCount == 1U, "flap penalty reset clear pending armed");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyArmCount == 1U, "flap penalty reset clear arm count armed");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyClearCount == 0U, "flap penalty reset clear ordinary clear idle before reset");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyBypassClearCount == 0U, "flap penalty reset clear bypass clear idle before reset");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyResetClearCount == 0U, "flap penalty reset clear reset clear idle before reset");
	vAssertTrue(xResult.uPreferredRecoveryHoldoffTargetCount == 3U, "flap penalty reset clear penalty target armed");

	xState.uIsAvailable = 1U;
	vAssertTrue(
		rsrx_channel_manager_update_channel(&xContext, 0U, &xState) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty reset clear primary restored before reset");
	vAssertTrue(
		rsrx_channel_manager_reset(&xContext) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty reset clear reset");
	vAssertTrue(
		rsrx_channel_manager_select_channel(&xContext, &xResult) == RSRX_CHANNEL_MANAGER_STATUS_OK,
		"flap penalty reset clear select after reset");
	vAssertTrue(xResult.eSelectedChannelId == RSRX_TRANSPORT_CHANNEL_PRIMARY, "flap penalty reset clear selected preferred after reset");
	vAssertTrue(xResult.uPreferredRecoveryPendingPenaltyCount == 0U, "flap penalty reset clear pending cleared");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyArmCount == 1U, "flap penalty reset clear arm count retained");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyClearCount == 0U, "flap penalty reset clear ordinary clear retained");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyBypassClearCount == 0U, "flap penalty reset clear bypass clear retained");
	vAssertTrue(xResult.uPreferredRecoveryPenaltyResetClearCount == 1U, "flap penalty reset clear count incremented");
