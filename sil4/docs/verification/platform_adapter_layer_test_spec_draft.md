...
| TC-PA-002 | SR-003 | transport encode 및 timer action 변환 검증 | transport send stub, codec encode 구현, monotonic clock stub 준비 | `START_HANDSHAKE`, `DELIVER_DATA`, `START_SUPERVISION_TIMER` dispatch | encoded send request 및 timer command가 생성 | message type/reason/payload, timer id/command/deadline이 설계와 일치 |
