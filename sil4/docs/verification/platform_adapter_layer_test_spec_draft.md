...
| TC-PA-012 | FR-003, SR-003 | rejected inbound record side-effect 차단 검증 | outstanding application send가 존재하고 protocol context last rx가 초기값인 transport adapter 준비 | sequence gap inbound message를 `rsrx_transport_adapter_record_inbound_message`로 직접 record | outstanding send, inbound cache, clear-on-inbound/deferred-dispatch telemetry가 변경되지 않는다 | adapter layer가 protocol context record rejection을 무시해 stale/gap frame을 정상 inbound progress로 취급하지 않는다 |
| TC-PA-013 | FR-003, IF-002, SR-003 | transport adapter query output guard 검증 | stale channel state와 invalid context 또는 topology-mismatch query fault 준비 | `rsrx_transport_adapter_query_channel` 호출 | `INVALID_ARGUMENT` 또는 `RX_ERROR` 반환 시 output channel state가 `INVALID/0` baseline으로 clear된다 | query 실패 경로가 caller에게 이전 channel availability를 새 결과처럼 남기지 않는다 |
| TC-PA-014 | FR-003, IF-002, SR-003 | transport adapter receive output guard 검증 | stale transport frame과 invalid context 준비 | `rsrx_transport_adapter_receive_frame` 호출 | `INVALID_ARGUMENT` 반환 시 output frame이 `INVALID/null/0/NONE` baseline으로 clear된다 | receive 실패 경로가 caller에게 이전 frame payload/event를 새 수신 결과처럼 남기지 않는다 |
| TC-PA-015 | IF-002, FR-007, SR-004 | executor table builder output guard 검증 | stale executor table과 invalid external executor 준비 | `rsrx_platform_adapter_build_executor_table` 호출 | `INVALID_ARGUMENT` 반환 시 executor table의 모든 context/dispatch slot이 null baseline으로 clear된다 | table build 실패 후 이전 dispatch binding을 새 platform binding처럼 사용할 수 없다 |
