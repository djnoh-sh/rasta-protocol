...
| TC-CHM-016 | FR-003, SR-003, IF-002 | preferred recovery threshold closeout matrix 검증 | holdoff threshold `3`/`4`와 flap-reset parity를 모두 재현 가능한 config 준비 | threshold `3`, threshold `4`, threshold `3 + flap-reset`, threshold `4 + flap-reset` matrix를 closeout wrapper로 실행 | higher-threshold hysteresis 규칙군에서 pure threshold growth와 flap-reset interaction이 서로 모순 없이 유지된다 | representative wrapper가 higher-threshold hysteresis 규칙군의 unit coverage를 하나의 closeout 항목으로 추적 가능하게 유지한다 |
