...
| TC-SUP-015 | FR-003, SR-002 | channel down failover 검증 | `ESTABLISHED` 상태 session, active-standby channel manager, primary down/secondary up 상태 준비 | `CHANNEL_DOWN(primary)` transport event 처리 | `IGNORED_EVENT` 반환, active channel이 secondary로 전환됨 | session state 유지, selected channel, failover decision, send failure budget reset telemetry, supervisor channel switch telemetry가 설계와 일치 |
| TC-SUP-017 | FR-003, SR-002 | channel up refresh 검증 | `ESTABLISHED` 상태 session, failover로 secondary active 상태, primary restored 상태 준비 | `CHANNEL_UP(primary)` transport event 처리 | `IGNORED_EVENT` 반환, active selection이 refresh되어 primary 복귀 | session state 유지, selected channel, decision telemetry, switch telemetry가 설계와 일치 |
