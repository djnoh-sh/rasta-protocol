...
| TC-PC-019 | FR-003 | unsequenced message family pass-through matrix 검증 | steady-state, post-recovery, retransmission pending protocol context와 `CONNECT_REQUEST`, `DISCONNECT`, `DIAGNOSTIC` inbound family | family마다 다양한 sequence/confirmation/protocol context 값을 가진 inbound message를 표 기반으로 순차 판정 | unsequenced inbound family는 sequence/confirmation ordering을 거치지 않고 각자의 suggested event를 그대로 유지한다 | current non-sequenced inbound family가 protocol ordering contract 바깥에서 deterministic pass-through semantics를 유지함을 닫는다 |
