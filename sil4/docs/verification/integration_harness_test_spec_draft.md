...
| TC-INT-030 | FR-004, SR-002 | repeated gap retransmission integration 검증 | handshake 완료 후 repeated gap frame이 scripted된 session-supervisor 준비 | 첫 gap으로 retransmission pending 진입 후 더 높은 gap frame을 추가 수신 | 상태는 `RETRANSMISSION_PENDING`을 유지하고 retransmission request가 다시 송신된다 | effective event, retransmission reason, repeated request count, state retention이 설계와 일치 |
