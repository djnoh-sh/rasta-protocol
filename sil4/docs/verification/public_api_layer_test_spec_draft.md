...
| TC-API-020 | IF-001, IF-002, SR-004 | public API critical-section balanced entry/exit 검증 | initialized session과 critical-section counting stub 준비 | start/connect/event/get-state/send/query-channel/receive-frame/reset public API 호출 | 각 guarded public API 호출에서 enter/exit count가 균형을 이루고 active depth가 0으로 복귀한다 | shared session-state 접근 경로와 transport boundary access 경로가 portable critical-section seam을 통과하며 host baseline에서 non-nested balanced 호출을 보인다 |
...
| TC-API-028 | IF-001, IF-002, SR-004 | transport channel query API guard 검증 | initialized session, channel query stub, critical-section failure stub 준비 | `rsrx_session_query_channel_state` 호출 | 성공 시 channel state/status가 caller buffer로 반환되고, invalid/enter failure 시 output state가 invalid/unavailable baseline으로 clear된다 | supervisor/target code가 transport adapter query internals를 직접 읽지 않고 bounded public API boundary를 사용할 수 있다 |
| TC-API-029 | IF-001, IF-002, SR-004 | transport frame receive API guard 검증 | initialized session, receive stub, critical-section failure stub 준비 | `rsrx_session_receive_transport_frame` 호출 | 성공 시 transport frame/status가 caller buffer로 반환되고, invalid/enter failure 시 output frame이 invalid/null/0/none baseline으로 clear된다 | supervisor/target code가 transport adapter receive internals를 직접 호출하지 않고 bounded public API boundary를 사용할 수 있다 |
