...
| TC-API-013 | FR-003, IF-001 | session reset transport-adapter runtime state 검증 | established session에서 outstanding send와 deferred send가 존재하는 상태 | `rsrx_session_reset` 호출 | outstanding/deferred outbound state와 inbound cache가 clear되고 runtime-reset telemetry가 증가한다 | public API reset 이후 stale outbound queue/protocol runtime state가 남지 않고 reset-origin이 관찰 가능하다 |
