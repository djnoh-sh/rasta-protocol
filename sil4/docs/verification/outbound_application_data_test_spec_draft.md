...
| TC-OUT-005 | FR-003, IF-001 | single outstanding send guard 검증 | `ESTABLISHED` 상태 session, 첫 outbound send 수행됨 | 다시 `rsrx_session_send_application_data` 호출 | `REJECTED` 반환 | queue 없이 outstanding send 1건만 허용한다 |
