...
| TC-OUT-010 | FR-003, IF-001 | deferred queue mixed clear long-run ordering 검증 | transport adapter 초기화 완료, `outstanding 1 + deferred 2` queue에서 alternating clear-source cycle을 만들 수 있는 상태 준비 | first cycle에서 feedback clear 후 inbound clear, second cycle에서 inbound clear 후 feedback clear를 수행 | alternating clear-source cycle 전반에서도 deferred two-slot queue는 FIFO ordering을 유지하고 clear-source telemetry가 누적된다 | dispatch count, clear-on-feedback count, clear-on-inbound count, last-request payload ordering이 설계와 일치 |
