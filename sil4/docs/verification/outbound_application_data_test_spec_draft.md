...
| TC-OUT-005 | FR-003, IF-001 | bounded queue guard 검증 | `ESTABLISHED` 상태 session, 첫 outbound send 수행됨 | second/third/fourth/fifth/sixth/seventh/eighth/ninth send 후 tenth send를 호출 | second/third/fourth/fifth/sixth/seventh/eighth/ninth send는 queue에 수용되고, tenth send는 `REJECTED`와 API callback/diagnostic rejection을 발생 | `outstanding 1 + deferred 8`를 넘으면 overflow reject가 관찰 가능하게 남는다 |
...
| TC-OUT-014 | FR-003, IF-001 | outbound queue/backpressure closeout matrix 검증 | `outstanding 1 + deferred 8` queue, alternating clear-source, overflow/busy reject accumulation, telemetry accumulation을 모두 재현 가능한 adapter test set 준비 | mixed clear long-run, manual/inbound reset-source, telemetry accumulation, overflow/busy accumulation matrix를 closeout wrapper로 실행 | bounded queue policy의 대표 unit variant에서 FIFO/depth parity, peak deferred depth retention, busy reject reset-source parity, overflow/busy accumulation parity가 서로 모순 없이 유지된다 | representative wrapper가 outbound queue/backpressure 규칙군의 unit coverage를 하나의 closeout 항목으로 추적 가능하게 유지한다 |
