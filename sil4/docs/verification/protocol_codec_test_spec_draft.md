...
| TC-CODEC-019 | SR-001 | reserved header byte matrix reject 검증 | reserved header offsets `2`, `3`, `14`, `15` 중 하나가 non-zero인 otherwise well-formed frame 준비 | decode 수행 | 각 frame이 `DECODE_ERROR` 반환 | 모든 reserved header 위치가 zero-baseline 위반 시 수용되지 않음 |
