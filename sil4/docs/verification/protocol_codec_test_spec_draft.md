...
| TC-CODEC-009 | SR-001 | decode trailing bytes reject 검증 | declared payload length는 zero이나 frame에 trailing byte가 붙은 frame 준비 | decode 수행 | `LENGTH_MISMATCH` 반환 | declared length와 actual frame length가 일치하지 않는 trailing data를 generic decode error와 구분해 수용하지 않음 |
| TC-CODEC-010 | SR-001 | decode truncated payload reject 검증 | declared payload length는 one이나 actual frame에는 payload byte가 없는 frame 준비 | decode 수행 | `LENGTH_MISMATCH` 반환 | declared length보다 짧은 truncated frame을 generic decode error와 구분해 수용하지 않음 |
...
| TC-CODEC-012 | FR-004 | encode oversized payload length reject 검증 | encode request payload length가 `D_RSRX_CODEC_MAX_PAYLOAD_BYTES + 1`인 request 준비 | encode 수행 | `PAYLOAD_TOO_LARGE` 반환 | bounded codec frame capacity를 초과하는 outbound payload를 unsupported-message와 구분해 직렬화하지 않음 |
...
| TC-CODEC-020 | FR-003, FR-004, SR-001 | unsupported reason code reject 검증 | reason byte/code가 defined `rsrx_reason_code_t` 범위 밖인 encode request 및 otherwise well-formed frame 준비 | encode/decode 수행 | encode는 `UNSUPPORTED_MESSAGE`, decode는 `DECODE_ERROR` 반환 | wire/header reason field가 정의되지 않은 상태 이유로 상위 계층에 전달되지 않음 |
...
| TC-CODEC-022 | FR-003, SR-001 | non-frame transport event decode reject 검증 | `eEventType`이 `FRAME_RECEIVED`가 아닌 otherwise well-formed frame 준비 | decode 수행 | `DECODE_ERROR` 반환 | codec decode가 channel/sender lifecycle event를 payload frame으로 오인하지 않음 |
