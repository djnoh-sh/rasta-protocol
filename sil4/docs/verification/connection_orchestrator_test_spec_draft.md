...
| --- | --- | --- | --- | --- | --- | --- |
| TC-OR-001 | FR-001, IF-001 | orchestrator 초기화 및 상태 조회 검증 | 유효 action sink 준비 | init 호출 | 상태가 `UNINITIALIZED` | init 성공 및 상태 일치 |
...
| TC-OR-004 | IF-001, SR-004 | invalid argument 방어 검증 | null context 또는 null sink callback | init/process 호출 | 정의된 invalid argument 처리 | deterministic report 또는 오류 반환 |
