...
| --- | --- | --- | --- | --- | --- | --- |
| TC-PLAT-001 | IF-002 | platform header contract 검증 | header include 가능 | contract test build/run | clock/timer/diagnostics/critical-section port 타입이 정상 사용 가능 | compile 및 실행 성공 |
...
| TC-PLAT-004 | IF-002, SR-003 | critical-section port 구조체 계약 검증 | 없음 | critical-section port 인스턴스 생성 | context, enter, exit callback 필드 접근 가능 | SafeRTOS/task/ISR binding을 위한 locking seam이 platform contract에 존재한다 |
