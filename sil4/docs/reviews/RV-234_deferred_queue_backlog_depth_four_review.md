...
3. `R-004` residual이 current configured depth `4` 이후의 backlog/fairness/runtime-feedback growth로만 읽히도록 roadmap/traceability/review wording이 함께 좁혀졌는지 확인한다.
...
3. spec, roadmap, traceability linkage가 current bounded queue policy를 `outstanding 1 + deferred 4`로 읽도록 다시 정렬됐다.
   - `TC-OUT-014`와 `TC-INT-100`이 `RV-234`와 직접 연결돼 depth `4` closeout review를 가리킨다.

## Verdict

- Pass. current bounded queue policy는 `outstanding 1 + deferred 4` 기준으로 일관되게 정리됐다.
- Follow-up residual은 current configured depth `4` 이후의 deeper backlog policy, beyond-FIFO fairness, queue-growth 이후 runtime-feedback growth로 유지한다.
