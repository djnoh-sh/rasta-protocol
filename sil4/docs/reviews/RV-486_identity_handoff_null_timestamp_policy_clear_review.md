# Review Record - RV-486 Identity Handoff Null Timestamp Policy Clear
...
- `TC-CODEC-045` now verifies that a null timestamp policy returns `RSRX_CODEC_STATUS_INVALID_ARGUMENT`.
...
Accepted as identity+timestamp handoff evidence for null timestamp-policy output clear behavior.
