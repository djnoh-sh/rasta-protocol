...
1. `TC-CODEC-022` verifies that decode rejects an otherwise well-formed payload with `NON_FRAME_EVENT` when the transport event type is not `RSRX_TRANSPORT_EVENT_FRAME_RECEIVED`.
