...
- 최근 업데이트: `EVID-CI-120`/`RV-520`로 operational evidence readiness를 재확인했고, baseline fetch는 available/closed, vendor export는 missing 상태임을 현재 R-005 snapshot으로 고정했다.
- 다음 주력 단계: `actual vendor export acquisition`, `AM263Px/SafeRTOS target artifact acquisition`, controlled requirement가 선택한 checksum/security policy만 추가 구현
...
| Safety Evidence | In Progress | cppcheck reports, stack/memory runbook/helper, strict-warning policy, operational evidence snapshots `EVID-CI-108`/`EVID-CI-120`, V&V v1.6 official response, comprehensive V&V 2026-06-02 response including Finding G corrective action, `EVID-TGT-001`, `EVID-TGT-003`, portable API concurrency/timer-quiescence closeout `RV-445` | actual vendor finding export, target-qualified stack/memory-map artifacts, target concurrency evidence |
| AM263Px / SafeRTOS Porting | Planned | `EVID-TGT-001`, `EVID-TGT-003`, `RV-415`, `RV-519`; portable core/adapter boundary is ready; target-specific layer intentionally not implemented in core | completed SafeRTOS binding artifact, TI driver transport adapter, optional hardware-backed CRC/crypto adapter, target stack/memory/timing evidence |
...
- Latest document-only planning update is `RV-520`, which refreshes R-005 operational readiness after baseline closure and confirms vendor export remains the active blocker; no host verification rerun was required for that document-only step.
...
| AM263Px/SafeRTOS porting layer | Planned | target artifact execution pending; planning artifacts are `EVID-TGT-001` and `EVID-TGT-003` |
| AM263Px/SafeRTOS target evidence | Waiting | target hardware/build environment, completed `EVID-TGT-003`, SafeRTOS safety evidence, linker/stack/timing capture process, software-vs-hardware CRC equivalence and HW diagnostic capture process |
