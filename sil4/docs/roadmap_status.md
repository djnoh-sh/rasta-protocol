...
- 최근 업데이트: `RV-558`로 `SR-CHECKSUM-PARITY-001` design/test packet을 추가해 non-none SR checksum/hash 구현 착수 전 필요한 algorithm/profile selection, coverage, key/IV policy, vectors, status taxonomy, target HW/SW equivalence gate를 명확히 했다.
- 다음 주력 단계: external lane은 `actual vendor export acquisition` 및 `first AM263Px/SafeRTOS target package execution`; local lane은 drafted parity packets의 controlled inputs 확보이며, MAC/time/SR-checksum/redundancy-CRC/parallel-delivery 구현은 각 packet의 missing controlled inputs가 채워질 때까지 보류
...
| Traceability | In Progress | `sil4/docs/traceability/traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-558` | 새 policy/evidence growth마다 review/spec linkage 유지 |
...
- Changes from `RV-529` through `RV-558` are document-only closeout, wording, priority, V&V response, scope-decision, design/test packet, or roadmap-evidence clarifications unless explicitly noted otherwise; they did not change source or executable test logic.
...
- Latest document-only Parallel Delivery / multi-path merge design/test packet is `RV-556`, which adds `RED-MODE-PARITY-001` and keeps source implementation blocked until exact routing policy, duplicate detection key, send-feedback aggregation, diagnostics, and AM263Px/SafeRTOS target evidence inputs are supplied.
- Latest document-only CRC-bearing redundancy design/test packet is `RV-555`, which adds `RED-CRC-PARITY-001` and keeps source implementation blocked until exact option layout, CRC parameters, vectors, supervisor diagnostics, and target HW/SW equivalence inputs are supplied.
- Latest document-only dynamic time supervision design/test packet is `RV-554`, which adds `TIME-PARITY-001` and keeps source implementation blocked until exact timing policy, runtime ownership, vector, and target timing evidence inputs are supplied.
- Latest document-only MAC/security design/test packet is `RV-553`, which adds `MAC-PARITY-001` and keeps source implementation blocked until exact clause/profile/key/vector/evidence inputs are supplied.
- Latest document-only standard parity scope decision is `RV-552`, which adds `SCOPE-RASTA-001` and updates `INV-RASTA-001` so Section 9 items are full parity scope but remain gated on exact design/test packets before implementation.
- Latest document-only V&V parity-gap response is `RV-551`, which accepts 2026-06-19 comprehensive V&V Section 9 as standard-parity planning input for MAC, dynamic clock/time supervision, CRC-bearing redundancy, and Parallel Delivery; no host verification rerun was required for that document-only step.
...
- Latest document-only work-priority reassessment is `RV-530`, which confirms no additional unselected host-only implementation growth should proceed without external/target artifacts or a controlled scope/requirement/status/policy selection; no host verification rerun was required for that document-only step.
...
| R-003 Redundancy | Holdoff/flap parity through configured target `20`, terminal outcome unit/integration parity through `20`, topology validation, unsupported `REDUNDANT` topology rejection, public API guard rejection with stale selection result clear, switch audit, reset audit preservation, penalty telemetry including saturation boundary, stability wrappers, hysteresis integration traceability `RV-470`, long-run integration traceability `RV-471`, flap-bypass closeout wrapper alignment `RV-472`, and Parallel Delivery implementation gate `RV-556` are representative-closeout/planning-complete as applicable. | fill `RED-MODE-PARITY-001` routing, duplicate detection, send-feedback aggregation, diagnostics, and target evidence inputs before implementation | acquire controlled redundancy policy inputs |
...
| R-006 Codec/security | Current codec skeleton, CRC32 optional path, typed status taxonomy, selected CRC integration, calculator injection seam, policy gates, encode/decode stale-output representative evidence, decode null-argument output clear, CRC32 truncated-output clear, current CRC32 inner status preservation matrix representative-closeout `RV-529`, unsupported-message/reason output clear including out-of-range disconnect reason mapping, trailing/truncated/oversized payload output clear, direct misuse event/channel output clear, `INV-RASTA-001` inventory, `PDU-PARITY-001` SR wire-profile draft, `SCOPE-RASTA-001` standard parity decision, `MAC-PARITY-001`, `TIME-PARITY-001`, `RED-CRC-PARITY-001`, `SCI-SCOPE-001`, and `SR-CHECKSUM-PARITY-001`, profile/packet/mapping contracts, fixed big-endian SR byte-order helpers, no-checksum SR common-header/payload encode/decode including encode/decode guard, payload-boundary, supported control-family, control wire-type byte, hand-authored control fixture decode, payload-bearing data fixture decode, and selected no-checksum parity reassessment evidence `RV-506`/`RV-507`/`RV-508`/`RV-509`/`RV-512`/`RV-515`/`RV-516`/`RV-517`/`RV-518`, selected no-checksum default profile, checksum profile unsupported-rejection boundary, unsupported `INVALID`/`RetrResp`/`RetrData` numeric mapping clear behavior, codec-level static timestamp/window admission boundary including confirmed timestamp stale and causal-order guards `RV-477`/`RV-494`, confirmed timestamp handoff clear evidence `RV-478`/`RV-485`, timestamp-admitted handoff bounded-payload evidence `RV-495`, identity+timestamp handoff reject propagation evidence `RV-496`, identity+timestamp handoff clear evidence `RV-479`/`RV-483`/`RV-484`/`RV-486`/`RV-487`, timestamp-admitted handoff mapping, supervisor runtime SR selection, receiver/sender identity admission boundary, supervisor identity policy wiring, redundancy PDU metadata profile, redundancy CRC option admission boundary, option A no-CRC redundancy PDU encode/decode behavior, carried no-checksum SR decode bridge with outer malformed/oversized and inner truncated/trailing status preservation plus supervisor/integration propagation `RV-497`/`RV-498`/`RV-499`/`RV-500`/`RV-501`/`RV-502`, supervisor redundancy-carried SR runtime selection, redundancy-carried SR integration handoff, post-closeout priority reassessment `RV-503`, V&V Section 9 parity-gap planning response `RV-551`, standard parity scope decision `RV-552`, MAC/security implementation gate `RV-553`, dynamic time supervision implementation gate `RV-554`, CRC-bearing redundancy implementation gate `RV-555`, SCI/application aggregation scope decision `RV-557`, and SR checksum implementation gate `RV-558` are in place. | fill `MAC-PARITY-001`, `TIME-PARITY-001`, `RED-CRC-PARITY-001`, and `SR-CHECKSUM-PARITY-001` controlled inputs; SCI/application aggregation remains excluded unless reopened by controlled input | acquire controlled codec/security inputs |
...
| RaSTA PDU/checksum/timestamp parity | Representative Host Closed | selected no-checksum SR host parity is closed by `RV-506..RV-518`; exact official/customer clause mapping, non-none checksum algorithm implementation, dynamic clock/time supervision, and CRC-bearing redundancy behavior remain standard-parity decisions |
| Non-none SR checksum/hash profiles | Design/Test Packet Drafted / Inputs Missing | selected no-checksum SR behavior remains valid; `SR-CHECKSUM-PARITY-001` lists missing profile selection, coverage, key/IV policy, vectors, status taxonomy, and target HW/SW equivalence evidence |
| RaSTA SCI/application-message aggregation | Scoped Out / Controlled Reopen Required | current SR data field is treated as bounded payload bytes; `SCI-SCOPE-001` excludes SCI-P, SCI-LS, and `RastaMessageData` multi-message aggregation until official/customer scope input reopens it |
| MAC/security extension | Design/Test Packet Drafted / Inputs Missing | current baseline correctly rejects `uRequireMac`; `MAC-PARITY-001` lists missing clause mapping, algorithm/key lifecycle, coverage, wire placement, vectors, and target evidence boundary |
| Dynamic Clock/Time Supervision | Design/Test Packet Drafted / Inputs Missing | current static timestamp admission is not full dynamic T_max/drift/retransmission-delay supervision; `TIME-PARITY-001` lists missing timing clauses, T_max/drift parameters, retransmission-delay budget, runtime ownership, and target timing evidence |
| CRC-bearing redundancy PDU behavior | Design/Test Packet Drafted / Inputs Missing | current option A no-CRC redundancy behavior remains valid; `RED-CRC-PARITY-001` lists missing option layout, CRC parameters, vectors, supervisor diagnostics, and target HW/SW equivalence evidence |
| Future redundancy routing modes | Design/Test Packet Drafted / Inputs Missing | policy decision beyond current active-standby baseline; `RED-MODE-PARITY-001` lists missing routing policy, duplicate detection key, send-feedback aggregation, diagnostics, and target evidence |
...
4. `Fill SR-CHECKSUM-PARITY-001 Controlled Inputs When Official/Customer Checksum Data Is Available`
5. `Fill RED-MODE-PARITY-001 Controlled Inputs When Official/Customer Redundancy Policy Is Available`
6. `Fill RED-CRC-PARITY-001 Controlled Inputs When Official/Customer CRC Data Is Available`
7. `Fill MAC-PARITY-001 Controlled Inputs When Official/Customer Data Is Available`
8. `Fill TIME-PARITY-001 Controlled Inputs When Official/Customer Timing Data Is Available`
9. `Reopen SCI-SCOPE-001 Only If Official/Customer Scope Requires SCI/Application Aggregation`
...
- 통과 조건:
  ...
  - 2026-06-19 V&V Section 9 parity gaps are tracked as standard-parity decisions and require design/test packets before source implementation
  - numeric parity growth는 current target range에서 closeout으로 유지하고, 새 policy/failure-mode가 있을 때만 확장할 것
  - unselected host-only growth는 `RV-530` 기준으로 중단하고, 다음 구현은 external/target artifact 또는 controlled scope/requirement/status/policy selection으로만 열 것
