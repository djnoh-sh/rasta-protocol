...
- 최근 업데이트: `TC-SUP-073`/`RV-388`로 supervisor init이 stale switch/holdoff audit telemetry를 neutral baseline으로 clear하는 contract를 고정했다.
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-388` | 새 policy growth마다 review/spec linkage 유지 |
...
| Transport Supervisor | In Progress | `TC-SUP-001..073`, runtime/channel/codec-status reports, public API guard evidence `RV-380`, init switch-audit baseline evidence `RV-388` | richer runtime fault-ordering variants |
...
- Latest local verification was green after `RV-388`.
