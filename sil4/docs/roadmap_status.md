...
| P4 | 통합/강건성 검증 | integration harness, 장시간/경계/고장주입 시험 | In Progress | 96% | current P4는 integration/matrix representative family 기준으로 closeout 상태다. redundancy 쪽은 threshold-aware terminal outcome direct coverage `3..16`, switch-audit envelope, broader stability representative wrapper set까지 정리돼 있고, runtime/sequencing 쪽도 ordering representative family, runtime feedback representative wrapper set, queue representative wrapper set, queue report representative matrix, budget scope closeout이 traceability/roadmap/review까지 연결된 상태다. 따라서 phase residual은 current family inventory 추가가 아니라 broader longer-run redundancy generalization, current envelope 밖의 switch-audit/runtime-fault policy growth, richer sequencing/runtime fault-ordering variant 쪽에 더 가깝다 |
...
| R-004 | outbound application send의 다음 queue policy 단계가 아직 열려 있음 | current bounded `outstanding 1 + deferred 2` queue family는 queue/backpressure closeout matrix, queue representative wrapper set, queue report representative matrix 기준으로 representative closeout 상태다. 따라서 residual은 current queue report/long-run family 내부 gap이 아니라 deeper backlog policy, current FIFO를 넘는 fairness policy, queue-growth 이후 retry/runtime feedback semantics를 `R-002`와 구분해 확장하는 next queue policy growth 쪽에 더 가깝다 | next queueing policy와 runtime feedback contract 설계 |
