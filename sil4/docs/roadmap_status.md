...
- 최근 업데이트: `TC-INT-217`/`RV-514`로 retransmission-pending 상태의 unsequenced `CONNECT_REQUEST` ordering-pass-through 및 current conservative fail-safe policy evidence를 추가했다.
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-514` | 새 policy/evidence growth마다 review/spec linkage 유지 |
...
- Latest local verification was green after `RV-514`.
