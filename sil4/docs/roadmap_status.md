...
- 최근 업데이트: `EVID-TGT-007`/`RV-522`로 현재 진행 가능한 일의 우선순위를 재평가했고, 외부 artifact/controlled requirement 대기 항목과 분리해 callback reentrancy/deferred-callback policy template을 local-action lane으로 추가했다.
- 다음 주력 단계: external lane은 `actual vendor export acquisition` 및 `first AM263Px/SafeRTOS target package acquisition`; local lane은 `callback reentrancy/deferred-callback policy target fill-in readiness`, 그 외 checksum/security 구현은 controlled requirement 선택 시에만 진행
...
| Safety Evidence | In Progress | cppcheck reports, stack/memory runbook/helper, strict-warning policy, operational evidence snapshots `EVID-CI-108`/`EVID-CI-120`, V&V v1.6 official response, comprehensive V&V 2026-06-02 response including Finding G corrective action, `EVID-TGT-001`, `EVID-TGT-003`, `EVID-TGT-005`, `EVID-TGT-007`, portable API concurrency/timer-quiescence closeout `RV-445` | actual vendor finding export, target-qualified stack/memory-map artifacts, target concurrency evidence |
| AM263Px / SafeRTOS Porting | Planned | `EVID-TGT-001`, `EVID-TGT-003`, `EVID-TGT-005`, `EVID-TGT-007`, `RV-415`, `RV-519`, `RV-521`, `RV-522`; portable core/adapter boundary is ready; target-specific layer intentionally not implemented in core | completed SafeRTOS binding artifact, completed callback policy artifact, TI driver transport adapter, optional hardware-backed CRC/crypto adapter, target stack/memory/timing evidence |
...
- Latest document-only planning update is `RV-522`, which reassesses currently actionable work and adds the AM263Px/SafeRTOS callback reentrancy/deferred-callback policy template; no host verification rerun was required for that document-only step.
...
| AM263Px/SafeRTOS porting layer | Planned | target artifact execution pending; planning artifacts are `EVID-TGT-001`, `EVID-TGT-003`, `EVID-TGT-005`, and `EVID-TGT-007` |
...
2. `External: First AM263Px/SafeRTOS Target Package Acquisition`
3. `Local Prep: Fill Target Callback Reentrancy Policy When Target Context Is Known`
...
6. `Selected Requirement: MAC/Security Extension`
7. `Selected Requirement: Protocol Sequencing Variants For New Message/Ordering Family`
8. `Selected Requirement: Redundancy Next Policy Growth`
