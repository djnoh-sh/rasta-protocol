...
- 최근 업데이트: `RV-552`로 V&V Section 9 이후의 `SCOPE-RASTA-001` 표준 parity scope decision을 추가해 MAC, dynamic clock/time supervision, CRC-bearing redundancy, Parallel Delivery를 full parity scope로 수용하되 구현 전 design/test packet gate를 요구하도록 정리했다.
- 다음 주력 단계: external lane은 `actual vendor export acquisition` 및 `first AM263Px/SafeRTOS target package execution`; local lane은 target context가 들어오면 `EVID-TGT-003`/`EVID-TGT-005`/`EVID-TGT-007` fill-in 지원, 그 외 MAC/security, dynamic time supervision, CRC-bearing redundancy, Parallel Delivery는 각 design/test packet이 exact clause/vector/evidence boundary를 정의한 뒤에만 구현
...
| Traceability | In Progress | `sil4/docs/traceability/traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-552` | 새 policy/evidence growth마다 review/spec linkage 유지 |
...
- Changes from `RV-529` through `RV-552` are document-only closeout, wording, priority, V&V response, scope-decision, or roadmap-evidence clarifications unless explicitly noted otherwise; they did not change source or executable test logic.
...
| MAC/security extension | Design/Test Packet Needed | current baseline correctly rejects `uRequireMac`; `SCOPE-RASTA-001` accepts MAC as full parity scope, but implementation waits for algorithm/key lifecycle, vectors, and evidence boundary |
...
4. `Selected Design/Test Packet: MAC/Security Sign/Verify`
5. `Selected Design/Test Packet: Dynamic Clock/Time Supervision`
6. `Selected Design/Test Packet: CRC-Bearing Redundancy PDU Behavior`
7. `Selected Design/Test Packet: Parallel Delivery / Multi-Path Merge`
