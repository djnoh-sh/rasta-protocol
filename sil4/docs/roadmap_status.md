...
- 최근 업데이트: `TC-API-025`/`TC-SUP-082`/`RV-440`으로 supervisor inbound event resolution이 protocol-context internals 직접 읽기 대신 session public API critical-section boundary를 사용하도록 갱신했다.
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-440` | 새 policy/evidence growth마다 review/spec linkage 유지 |
...
| Adapter Layer / Public API | In Progress | `rsrx_platform_adapters.*`, `rsrx_api.*`, `TC-PA-010..015`, `TC-API-003`, `TC-API-014..025`, public API report-output guard evidence `RV-427`, adapter query/receive/executor output guard evidence `RV-429..RV-431`, reset timer quiescence evidence `RV-432`, critical-section startup gate evidence `TC-CFG-011`/`RV-433`, public API critical-section guard evidence `RV-434`, outbound telemetry snapshot evidence `RV-435`, outbound queue snapshot/report-helper evidence `RV-436`/`RV-439`, channel-manager snapshot evidence `RV-437`, inbound event resolve boundary evidence `RV-440` | target runtime binding, callback reentrancy policy, selected codec policy 유지 |
...
| Transport Supervisor | In Progress | `TC-SUP-001..082`, `TC-INT-210`, runtime/channel/codec-status/SR-runtime/identity/redundancy-SR reports, public API guard evidence `RV-380`/`RV-425`, init switch/runtime-loop baseline evidence `RV-388..RV-389`, SR runtime wiring evidence `RV-408`, SR identity wiring evidence `RV-410`, redundancy-carried SR runtime wiring/integration evidence `RV-417..RV-418`, outbound queue snapshot refresh/helper evidence `RV-436`/`RV-439`, channel-manager snapshot refresh evidence `RV-437`, active-channel snapshot helper evidence `RV-438`, inbound event resolve boundary evidence `RV-440` | richer runtime fault-ordering variants |
...
- Latest local verification was green after `RV-440`.
...
| Public API critical-section policy | Started | portable contract/startup gate `RV-433`, public API guard application `RV-434`, telemetry snapshot API `RV-435`, outbound queue snapshot/report-refresh/helper evidence `RV-436`/`RV-439`, channel-manager snapshot/report-refresh evidence `RV-437`, active-channel snapshot helper evidence `RV-438`, and inbound event resolve boundary evidence `RV-440` are in place; SafeRTOS binding and callback reentrancy policy evidence still required |
