...
- 최근 업데이트: `RV-553`으로 `MAC-PARITY-001` design/test packet을 추가해 MAC/security 구현 착수 전 필요한 clause mapping, algorithm/key lifecycle, coverage, wire placement, vectors, target evidence gate를 명확히 했다.
- 다음 주력 단계: external lane은 `actual vendor export acquisition` 및 `first AM263Px/SafeRTOS target package execution`; local lane은 `Dynamic Clock/Time Supervision` design/test packet 작성이 다음 우선순위이며, MAC/security 구현은 `MAC-PARITY-001`의 missing controlled inputs가 채워질 때까지 보류
...
| Traceability | In Progress | `sil4/docs/traceability/traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-553` | 새 policy/evidence growth마다 review/spec linkage 유지 |
...
- Changes from `RV-529` through `RV-553` are document-only closeout, wording, priority, V&V response, scope-decision, design/test packet, or roadmap-evidence clarifications unless explicitly noted otherwise; they did not change source or executable test logic.
...
| Dynamic Clock/Time Supervision | Design/Test Packet Needed | current static timestamp admission is not full dynamic T_max/drift/retransmission-delay supervision; implementation waits for timing clauses, T_max/drift parameters, and timer-source policy |
...
4. `Selected Design/Test Packet: Dynamic Clock/Time Supervision`
5. `Selected Design/Test Packet: CRC-Bearing Redundancy PDU Behavior`
6. `Selected Design/Test Packet: Parallel Delivery / Multi-Path Merge`
