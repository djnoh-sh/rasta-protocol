...
- 현재 전체 진행률 추정: `85~88%`
- 현재 단계: `wider static analysis baseline cleanup 완료 단계`
- 다음 주력 단계: `transport supervisor completion`, `redundancy policy refinement`, `CI-linked evidence expansion`
...
| --- | --- | --- | --- |
| IP-001 | Public API hardening | timer ingress, outbound send, application delivery contract, single outstanding send guard, outbound telemetry, busy reject API/diagnostic correlation, repeated busy reject streak telemetry 포함 | queueing/backpressure policy와 callback/report semantics 결정 |
...
| NS-006 | Static Analysis and MISRA Evidence | SIL4 과제의 핵심 증빙 | severity mapping formalization과 CI linkage 확보 |
...
| R-004 | outbound application send가 direct-send 모델에 머묾 | single outstanding send guard, accept/reject telemetry, busy reject correlation, repeated reject streak telemetry는 들어갔지만 queue/backpressure/retry semantics는 아직 단순하다 | queueing policy와 runtime feedback contract 설계 |
