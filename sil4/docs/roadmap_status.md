...
- 최근 업데이트: `TC-CODEC-061`/`RV-526`로 CRC32 wrapper가 valid CRC 이후에도 truncated payload inner decode status와 stale-output clear contract를 보존함을 고정했다.
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-526` | 새 policy/evidence growth마다 review/spec linkage 유지 |
...
- Latest local verification was green after `RV-526`.
