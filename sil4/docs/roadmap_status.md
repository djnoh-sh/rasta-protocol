...
| IP-004 | Transport supervisor maturation | inbound decode handoff, sequence gate, poll receive, channel state gate 구현 | send result와 timer interaction 반영 |
...
| R-002 | transport supervisor 운영 루프 미완 | poll receive와 channel gate는 있으나 send result/timer interaction이 아직 비어 있음 | send result/timer interaction path 구현 |
