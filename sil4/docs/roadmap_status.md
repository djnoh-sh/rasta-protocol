...
- 최근 업데이트: `TC-CODEC-043`으로 RaSTA SR timestamp/window admission boundary를 추가해 zero/future/stale/regressed/confirmed-future cases를 typed status로 구분한다.
- 다음 주력 단계: `RaSTA SR timestamp session-handoff integration`, `receiver/sender authenticity checks`, `actual CI/vendor evidence execution`, `AM263Px/SafeRTOS porting evidence planning`, `redundancy next policy growth`, `protocol sequencing next parity growth`
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-406` | 새 policy growth마다 review/spec linkage 유지 |
...
| Codec / Security | In Progress | `TC-CODEC-001..043`, `TC-INT-206`, `TC-INT-208`, `RV-337`, `RV-354..RV-367`, `RV-369..RV-378`, `RV-382..RV-384`, `RV-390`, `INV-RASTA-001`, `PDU-PARITY-001A..001F`, `RV-399..RV-406` | RaSTA SR timestamp session-handoff integration, receiver/sender authenticity checks, selected checksum algorithm implementation if required, redundancy CRC/PDU parity, optional MAC/security extension taxonomy, vendor security vectors |
...
- Latest local verification was green after `RV-406`.
...
| R-006 Codec/security | Current codec skeleton, CRC32 optional path, typed status taxonomy, selected CRC integration, calculator injection seam, policy gates, encode/decode stale-output representative evidence, decode null-argument output clear, CRC32 truncated-output clear, CRC32 inner status preservation including transport metadata misuse, reserved-header tamper output clear, unsupported-message/reason output clear, trailing/truncated/oversized payload output clear, direct misuse event/channel output clear, `INV-RASTA-001` inventory, `PDU-PARITY-001` SR wire-profile draft, profile/packet/mapping contracts, fixed big-endian SR byte-order helpers, no-checksum SR common-header/payload encode/decode, checksum profile unsupported-rejection boundary, and codec-level timestamp/window admission boundary are in place. | timestamp admission integration into protocol/session handoff, receiver/sender authenticity checks, selected checksum algorithm implementation only if required, RaSTA redundancy CRC/PDU parity, optional MAC/security extension definition, additional tamper taxonomy, vendor-oriented security negative vectors | integrate timestamp admission into session/protocol flow |
...
| RaSTA PDU/checksum/timestamp parity | Started | exact official/customer RaSTA clause mapping, timestamp session-handoff policy, selected checksum algorithm implementation only if required |
...
4. `RaSTA SR Timestamp Session-Handoff Integration`
