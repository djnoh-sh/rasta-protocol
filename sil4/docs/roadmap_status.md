...
- 최근 업데이트: `TC-CHM-060`/`RV-381`로 preferred recovery flap-penalty target saturation boundary를 직접 고정했다.
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-381` | 새 policy growth마다 review/spec linkage 유지 |
...
- Latest local verification was green after `RV-381`.
...
| R-006 Codec/security | Current codec skeleton, CRC32 optional path, typed status taxonomy, selected CRC integration, policy gates, encode/decode stale-output representative evidence, decode null-argument output clear, CRC32 truncated-output clear, reserved-header tamper output clear, unsupported-message/reason output clear, trailing/truncated/oversized payload output clear, and direct misuse event/channel output clear are in place. | actual MAC/timestamp/PDU parity, additional tamper taxonomy, vendor-oriented security negative vectors | codec-security parity definition |
...
- 통과 조건:
  - `R-001..R-007`이 current closeout state와 실제 residual만 가리킬 것
