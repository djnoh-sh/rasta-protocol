...
- 최근 업데이트: `TC-API-026`/`TC-SUP-083`/`RV-441`로 supervisor accepted inbound message record path가 transport-adapter internals 직접 mutation 대신 session public API critical-section boundary를 사용하도록 갱신했다.
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-441` | 새 policy/evidence growth마다 review/spec linkage 유지 |
...
| Platform / Transport Abstraction | In Progress | `rsrx_platform.h`, `rsrx_transport.h`, contract tests, critical-section port contract `TC-PLAT-004`/`RV-433` | public API guard application, 포팅 시 target adapter evidence |
| Adapter Layer / Public API | In Progress | `rsrx_platform_adapters.*`, `rsrx_api.*`, `TC-PA-010..015`, `TC-API-003`, `TC-API-014..026`, public API report-output guard evidence `RV-427`, adapter query/receive/executor output guard evidence `RV-429..RV-431`, reset timer quiescence evidence `RV-432`, critical-section startup gate evidence `TC-CFG-011`/`RV-433`, public API critical-section guard evidence `RV-434`, outbound telemetry snapshot evidence `RV-435`, outbound queue snapshot/report-helper evidence `RV-436`/`RV-439`, channel-manager snapshot evidence `RV-437`, inbound event resolve/record boundary evidence `RV-440`/`RV-441` | target runtime binding, callback reentrancy policy, selected codec policy 유지 |
...
| Transport Supervisor | In Progress | `TC-SUP-001..083`, `TC-INT-210`, runtime/channel/codec-status/SR-runtime/identity/redundancy-SR reports, public API guard evidence `RV-380`/`RV-425`, init switch/runtime-loop baseline evidence `RV-388..RV-389`, SR runtime wiring evidence `RV-408`, SR identity wiring evidence `RV-410`, redundancy-carried SR runtime wiring/integration evidence `RV-417..RV-418`, outbound queue snapshot refresh/helper evidence `RV-436`/`RV-439`, channel-manager snapshot refresh evidence `RV-437`, active-channel snapshot helper evidence `RV-438`, inbound event resolve/record boundary evidence `RV-440`/`RV-441` | richer runtime fault-ordering variants |
...
| Safety Evidence | In Progress | cppcheck reports, stack/memory runbook/helper, strict-warning policy, operational evidence snapshot, V&V v1.6 official response, comprehensive V&V 2026-06-02 response, `EVID-TGT-001` | actual vendor finding export, target-qualified stack/memory-map artifacts, API concurrency/timer-quiescence closeout evidence |
...
- Latest local verification was green after `RV-441`.
...
| R-009 API concurrency and reset quiescence | 2026-06-02 comprehensive V&V audit `Finding E/F` is accepted. `Finding F` reset timer quiescence is closed by `TC-API-019`/`RV-432`; `RV-433` adds the portable critical-section port contract/startup gate; `RV-434` applies balanced critical-section guards to initialized public API shared-state paths; `RV-435` adds caller-owned outbound telemetry snapshot API; `RV-436` adds caller-owned outbound queue snapshot API and routes supervisor outbound queue report refresh through that snapshot boundary; `RV-437` adds caller-owned channel-manager snapshot API and routes supervisor switch/holdoff report refresh through that snapshot boundary; `RV-438` routes supervisor active-channel helper through the channel-manager snapshot boundary; `RV-439` routes supervisor outstanding-feedback match helper through the outbound queue snapshot boundary including outstanding channel; `RV-440` routes supervisor inbound event resolution through a session public API critical-section boundary; `RV-441` routes supervisor accepted inbound message recording through a session public API critical-section boundary. | SafeRTOS target binding evidence, callback reentrancy/deferred-callback policy, target integration/fault-injection logs, remaining concurrent supervisor/session paths if used from separate tasks | acquire target binding evidence before claiming multi-task/ISR concurrency safety |
...
| Public API critical-section policy | Started | portable contract/startup gate `RV-433`, public API guard application `RV-434`, telemetry snapshot API `RV-435`, outbound queue snapshot/report-refresh/helper evidence `RV-436`/`RV-439`, channel-manager snapshot/report-refresh evidence `RV-437`, active-channel snapshot helper evidence `RV-438`, and inbound event resolve/record boundary evidence `RV-440`/`RV-441` are in place; SafeRTOS binding and callback reentrancy policy evidence still required |
...
- 통과 조건:
  ...
  - `R-009` API concurrency/reset quiescence가 구현/검증 또는 명확한 target-porting boundary로 closeout될 것
