...
- 최근 업데이트: `TC-API-023`/`TC-SUP-078`/`RV-436`로 outbound queue state와 telemetry를 caller-owned snapshot으로 복사하고 supervisor outbound report refresh가 이 snapshot boundary를 사용하도록 갱신했다.
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-436` | 새 policy/evidence growth마다 review/spec linkage 유지 |
...
| Transport Supervisor | In Progress | `TC-SUP-001..078`, `TC-INT-210`, runtime/channel/codec-status/SR-runtime/identity/redundancy-SR reports, public API guard evidence `RV-380`/`RV-425`, init switch/runtime-loop baseline evidence `RV-388..RV-389`, SR runtime wiring evidence `RV-408`, SR identity wiring evidence `RV-410`, redundancy-carried SR runtime wiring/integration evidence `RV-417..RV-418`, outbound queue snapshot refresh evidence `RV-436` | richer runtime fault-ordering variants |
...
- Latest local verification was green after `RV-436`.
...
| Public API critical-section policy | Started | portable contract/startup gate `RV-433`, public API guard application `RV-434`, telemetry snapshot API `RV-435`, and outbound queue snapshot/report-refresh evidence `RV-436` are in place; SafeRTOS binding and callback reentrancy policy evidence still required |
