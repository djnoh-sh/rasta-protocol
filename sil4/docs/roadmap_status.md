...
| R-004 | outbound application send가 minimal bounded queue 모델에 머묾 | deferred queue telemetry, overflow reject, busy reject threshold escalation/reset, queue/backpressure closeout representative integration까지는 확보됐다. 다만 현재 모델은 여전히 `outstanding 1 + deferred 1`에 머물러 있어 deeper queue/backpressure/retry semantics는 아직 단순하다 | queueing policy와 runtime feedback contract 설계 |
