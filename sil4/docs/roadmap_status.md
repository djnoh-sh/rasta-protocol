...
- 최근 업데이트: `TC-CODEC-020` unsupported reason decode failure에서 stale decoded-message output clear를 직접 검증했다.
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-373` | 새 policy growth마다 review/spec linkage 유지 |
...
| Codec / Security | In Progress | `TC-CODEC-001..032`, `TC-INT-206`, `TC-INT-208`, `RV-337`, `RV-354..RV-367`, `RV-369..RV-373` | MAC/timestamp/PDU parity, additional tamper taxonomy, vendor security vectors |
...
- Latest local verification was green after `RV-373`.
...
| R-006 Codec/security | Current codec skeleton, CRC32 optional path, typed status taxonomy, selected CRC integration, policy gates, encode/decode stale-output representative evidence, decode null-argument output clear, CRC32 truncated-output clear, reserved-header tamper output clear, unsupported-message output clear, and unsupported-reason output clear are in place. | actual MAC/timestamp/PDU parity, additional tamper taxonomy, vendor-oriented security negative vectors | codec-security parity definition |
