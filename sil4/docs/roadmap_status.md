...
- 최근 업데이트: `RV-551`로 2026-06-19 comprehensive V&V Section 9 parity gap을 답변서와 계획에 반영해 MAC, dynamic clock/time supervision, CRC-bearing redundancy, Parallel Delivery를 표준 parity decision/growth 항목으로 명확히 분리했다.
- 다음 주력 단계: external lane은 `actual vendor export acquisition` 및 `first AM263Px/SafeRTOS target package execution`; local lane은 target context가 들어오면 `EVID-TGT-003`/`EVID-TGT-005`/`EVID-TGT-007` fill-in 지원, 그 외 MAC/security, dynamic time supervision, CRC-bearing redundancy, Parallel Delivery, aggregation scope, protocol/redundancy policy는 controlled scope/requirement/status/policy family 추가 시에만 진행
...
- Changes from `RV-529` through `RV-551` are document-only closeout, wording, priority, V&V response, or roadmap-evidence clarifications unless explicitly noted otherwise; they did not change source or executable test logic.
...
| MAC/security extension | Standard Parity Decision Needed | current baseline correctly rejects `uRequireMac`, but V&V Section 9 treats MAC as certification-relevant parity; implement if full RaSTA standard parity is selected |
| Dynamic Clock/Time Supervision | Standard Parity Decision Needed | current static timestamp admission is not full dynamic T_max/drift/retransmission-delay supervision |
...
4. `Standard Parity Decision: MAC/Security, Dynamic Clock/Time Supervision, CRC-Bearing Redundancy, Parallel Delivery Scope`
5. `Selected Requirement: MAC/Security Sign/Verify`
6. `Selected Requirement: Dynamic Clock/Time Supervision`
7. `Selected Requirement: CRC-Bearing Redundancy PDU Behavior`
...
10. `Selected Requirement: Parallel Delivery / Redundancy Next Policy Growth`
...
| Controlled requirement work | Waiting | explicit selection of non-none checksum, CRC-bearing redundancy, SCI/application aggregation, MAC/security, dynamic clock/time supervision, protocol sequencing variant, or Parallel Delivery redundancy policy growth |
...
- 통과 조건:
  ...
  - 2026-06-19 V&V Section 9 parity gaps are tracked as standard-parity decisions, not as already-closed host behavior
