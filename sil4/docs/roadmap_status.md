...
| --- | --- | --- | --- |
| IP-001 | Public API hardening | timer ingress, outbound send, application delivery contract, single outstanding send guard와 outbound telemetry 포함 | queueing/backpressure policy와 callback/report semantics 결정 |
...
| R-004 | outbound application send가 direct-send 모델에 머묾 | single outstanding send guard와 accept/reject telemetry는 들어갔지만 queue/backpressure/retry semantics는 아직 단순하다 | queueing policy와 runtime feedback contract 설계 |
