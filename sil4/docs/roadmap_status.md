...
- 최근 업데이트: `RV-542`로 outbound queue verification spec의 예전 `deferred 3` wording을 current `outstanding 1 + deferred 12` policy와 representative three-entry path 기준으로 정렬했다.
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-542` | 새 policy/evidence growth마다 review/spec linkage 유지 |
