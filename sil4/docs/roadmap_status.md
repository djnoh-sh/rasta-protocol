...
- 현재 전체 진행률 추정: `85~88%`
- 현재 단계: `MISRA evidence baseline formalization 단계`
- 다음 주력 단계: `tool-specific MISRA mapping 확장`, `second-tool baseline 검토`, `CI-linked evidence expansion`
...
| P5 | 인증 증빙 강화 | static analysis, MISRA evidence, review record, safety case 입력 생성 | In Progress | 68% | baseline 증빙 체계, cppcheck cleanup, outbound queue policy review, CI linkage, severity mapping, artifact retention, MISRA subset severity, tool-specific mapping baseline 확보 |
...
| NS-006 | Static Analysis and MISRA Evidence | SIL4 과제의 핵심 증빙 | second-tool linkage와 vendor rule ID level mapping 확보 |
...
| R-005 | 인증 증빙 자동화 부족 | CI linkage, severity mapping, artifact retention, MISRA subset severity, tool-specific mapping baseline은 생겼지만 second-tool linkage와 vendor rule mapping이 아직 없다 | second-tool baseline, vendor rule ID mapping 검토 |
