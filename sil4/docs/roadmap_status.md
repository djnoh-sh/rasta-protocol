...
- 최근 업데이트: `TC-API-018`/`RV-396`으로 public API reset 이후 same session context가 startup/connect cycle에 재진입할 수 있는 contract를 고정했다.
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-396` | 새 policy growth마다 review/spec linkage 유지 |
...
- Latest local verification was green after `RV-396`.
