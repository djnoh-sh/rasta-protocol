...
- 최근 업데이트: `RV-534`로 protocol codec LLD가 default skeleton path뿐 아니라 selected no-checksum RaSTA SR 및 option A redundancy-carried SR path를 현재 구현 baseline으로 설명하도록 정렬했다.
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-534` | 새 policy/evidence growth마다 review/spec linkage 유지 |
