...
- 최근 업데이트: `TC-CODEC-041`로 RaSTA SR no-checksum common-header/payload encode/decode path와 malformed-frame typed reject evidence를 추가했다.
- 다음 주력 단계: `RaSTA SR checksum/hash profile handling`, `actual CI/vendor evidence execution`, `AM263Px/SafeRTOS porting evidence planning`, `redundancy next policy growth`, `protocol sequencing next parity growth`, `transport supervisor runtime feedback growth`
...
| Traceability | In Progress | `traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-404` | 새 policy growth마다 review/spec linkage 유지 |
...
| Codec / Security | In Progress | `TC-CODEC-001..041`, `TC-INT-206`, `TC-INT-208`, `RV-337`, `RV-354..RV-367`, `RV-369..RV-378`, `RV-382..RV-384`, `RV-390`, `INV-RASTA-001`, `PDU-PARITY-001A..001D`, `RV-399..RV-404` | RaSTA SR checksum/hash profile handling, timestamp/admission parity, redundancy CRC/PDU parity, optional MAC/security extension taxonomy, vendor security vectors |
...
- Latest local verification was green after `RV-404`.
...
| R-006 Codec/security | Current codec skeleton, CRC32 optional path, typed status taxonomy, selected CRC integration, calculator injection seam, policy gates, encode/decode stale-output representative evidence, decode null-argument output clear, CRC32 truncated-output clear, CRC32 inner status preservation including transport metadata misuse, reserved-header tamper output clear, unsupported-message/reason output clear, trailing/truncated/oversized payload output clear, direct misuse event/channel output clear, `INV-RASTA-001` inventory, `PDU-PARITY-001` SR wire-profile draft, profile/packet/mapping contracts, fixed big-endian SR byte-order helpers, and no-checksum SR common-header/payload encode/decode are in place. | RaSTA SR checksum/hash parity, timestamp admission/window parity, receiver/sender authenticity checks, RaSTA redundancy CRC/PDU parity, optional MAC/security extension definition, additional tamper taxonomy, vendor-oriented security negative vectors | define selected checksum/hash policy or explicit unsupported-profile rejection |
...
| RaSTA PDU/checksum/timestamp parity | Started | exact official/customer RaSTA clause mapping, timestamp/window policy, checksum/hash profile selection |
...
4. `RaSTA SR Checksum/Hash Profile Handling`
