...
| R-004 | outbound application send의 다음 queue policy 단계가 아직 열려 있음 | current bounded `outstanding 1 + deferred 2` queue family는 queue/backpressure closeout matrix, queue representative wrapper set, queue report representative matrix, outbound queue fairness representative matrix, outbound queue long-run representative matrix 기준으로 representative closeout 상태다. 따라서 residual은 current outbound queue family 내부 representative gap이 아니라 deeper backlog policy, current FIFO를 넘는 fairness policy, queue-growth 이후 retry/runtime feedback semantics를 `R-002`와 구분해 확장하는 next queue policy growth 쪽에 더 가깝다 | next queueing policy와 runtime feedback contract 설계 |
