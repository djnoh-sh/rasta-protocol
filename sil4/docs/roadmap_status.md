...
- 최근 업데이트: `RV-555`로 `RED-CRC-PARITY-001` design/test packet을 추가해 CRC-bearing redundancy PDU Option B-E 구현 착수 전 필요한 option layout, CRC parameters, vectors, supervisor diagnostics, target HW/SW equivalence gate를 명확히 했다.
- 다음 주력 단계: external lane은 `actual vendor export acquisition` 및 `first AM263Px/SafeRTOS target package execution`; local lane은 `Parallel Delivery / multi-path merge` design/test packet 작성이 다음 우선순위이며, MAC/time/redundancy-CRC 구현은 각 packet의 missing controlled inputs가 채워질 때까지 보류
...
| Traceability | In Progress | `sil4/docs/traceability/traceability_matrix_initial.md`, `RV-030`, `RV-217..RV-326`, `RV-332..RV-555` | 새 policy/evidence growth마다 review/spec linkage 유지 |
...
| Redundancy / Channel Manager | In Progress | holdoff `2..20`, flap-reset `2..20`, terminal outcome unit/integration `3..20`, topology validation, unsupported `REDUNDANT` topology rejection, public API guard evidence `TC-CHM-059`/`RV-379`/`RV-426`, flap-penalty saturation evidence `TC-CHM-060`/`RV-381`, reset audit preservation `TC-CHM-061`/`RV-387` | Parallel Delivery / multi-path merge policy growth and longer-run target soak |
...
- Changes from `RV-529` through `RV-555` are document-only closeout, wording, priority, V&V response, scope-decision, design/test packet, or roadmap-evidence clarifications unless explicitly noted otherwise; they did not change source or executable test logic.
...
| Future redundancy routing modes | Not Started | policy decision beyond current active-standby baseline; V&V Section 9 names Parallel Delivery and receive-side multi-path merge/filtering |
...
4. `Selected Design/Test Packet: Parallel Delivery / Multi-Path Merge`
