...
`PDU-PARITY-001A..001C` are now implemented as profile metadata, public SR packet data contracts, and numeric type/disconnect reason mappings. The remaining open decision before behavioral SR encode/decode implementation is byte-order compatibility: the existing repo-source helpers serialize according to host endian, while SIL4 parity needs either an official fixed byte order, a documented compatibility profile, or captured deployment golden vectors.
