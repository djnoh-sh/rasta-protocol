...
7. `RED-PDU-PARITY-001`: Redundancy PDU profile metadata, option A no-CRC encode/decode behavior, and carried no-checksum SR decode bridging are defined; add CRC-bearing behavior only if selected.
