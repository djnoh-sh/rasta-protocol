...
| SR safety code/checksum | `RastaPacket.checksum`, `rasta_calculate_hash`, `rastahashing.h` | RaSTA Configurable | Current CRC32 wrapper is not the same as SR hash/checksum parity; no-checksum is accepted and MD4/BLAKE2b/SipHash profiles are explicitly rejected until implemented | Implement selected SR checksum profile calculation only if a controlled requirement selects one |
| SR checksum algorithms: MD4, BLAKE2b, SipHash-2-4 with 0/8/16 byte checksum options | `rasta_hash_algorithm`, `rasta_checksum_type` in `rastahashing.h` | RaSTA Configurable | `sil4` has admission contract for none/unsupported profiles; current CRC32 path is separate | Add actual algorithm abstraction and vectors when a selected profile is required |
...
| Packet authenticity: sender/receiver ID validation | `sr_message_authentic` in analysis docs | RaSTA Mandatory | Codec-level receiver/sender identity admission and identity+timestamp handoff guard exist | Wire deployment identity policy into supervisor runtime selection |
...
3. `PDU-PARITY-003`: Add sender/receiver RaSTA ID authenticity validation. Status: codec-level admission boundary implemented; supervisor policy wiring remains open.
...
`PDU-PARITY-001A..001F` are now implemented through profile metadata, public SR packet data contracts, numeric type/disconnect reason mappings, fixed big-endian byte-order policy, no-checksum SR common-header/payload encode/decode behavior, explicit unsupported checksum-profile admission rejection, codec-level timestamp/window admission boundary, timestamp-admitted handoff mapping, supervisor runtime SR selection, and codec-level receiver/sender identity admission. The remaining open parity work is actual selected checksum/hash calculation if required, supervisor identity policy wiring, target timestamp source refresh policy, and redundancy PDU parity if that wire mode is in scope.
