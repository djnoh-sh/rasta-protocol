...
`PDU-PARITY-001A..001F` are now implemented through profile metadata, public SR packet data contracts, numeric type/disconnect reason mappings, fixed big-endian byte-order policy, no-checksum SR common-header/payload encode/decode behavior, explicit unsupported checksum-profile admission rejection, codec-level timestamp/window admission boundary, timestamp-admitted handoff mapping, and supervisor runtime SR selection. The remaining open parity work is actual selected checksum/hash calculation if required, receiver/sender authenticity checks, target timestamp source refresh policy, and redundancy PDU parity if that wire mode is in scope.
