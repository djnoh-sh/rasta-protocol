...
| Timestamp and confirmed timestamp sequence validation | `RastaPacket.timestamp`, `confirmed_timestamp`, `sr_cts_in_seq`, `cur_timestamp` in analysis docs | RaSTA Mandatory for full SR parity | Codec-level timestamp admission, handoff mapping, and explicit supervisor runtime SR selection exist | Capture target monotonic source refresh policy and replay/window evidence in the target package |
