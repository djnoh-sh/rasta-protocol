...
This profile definition narrows `R-006` from a broad codec/security residual to a concrete SR PDU parity backlog. The no-checksum SR common-header/payload behavioral path, unsupported checksum-profile admission boundary, codec-level timestamp admission boundary, timestamp-admitted handoff mapping, and supervisor runtime SR selection are implemented. This does not claim MD4/BLAKE2b/SipHash calculation, receiver/sender authenticity admission, redundancy PDU, target timestamp-source binding, or MAC/security-extension completion.
