...
| `rsrx_channel_manager_context_t` | struct | runtime active channel, holdoff 상태, pending flap penalty, cumulative switch/unavailable/penalty arm/applied/clear count 보관 | no dynamic memory |
...
- 선택 정책:
  ...
  - preferred recovery holdoff가 적용 가능한 동안 selection result는 active flag, progress, pending penalty, penalty arm/applied/clear count, target, remaining count를 보고하며, target/remaining은 pending flap penalty가 있으면 증가된 effective holdoff target 기준으로 계산한다.
...
- flap penalty가 configured된 경우 selection result의 pending penalty/penalty arm/applied/clear count/holdoff target/remaining telemetry는 다음 preferred recovery cycle의 강화된 holdoff target과 cumulative arm/applied/clear history를 caller에 직접 노출해야 한다.
