...
- 선택 정책:
  ...
  - preferred recovery holdoff가 적용 가능한 동안 selection result는 active flag, progress, pending penalty, target, remaining count를 보고하며, target/remaining은 pending flap penalty가 있으면 증가된 effective holdoff target 기준으로 계산한다.
...
- flap penalty가 configured된 경우 selection result의 pending penalty/holdoff target/remaining telemetry는 다음 preferred recovery cycle의 강화된 holdoff target을 caller에 직접 노출해야 한다.
