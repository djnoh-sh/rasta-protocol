...
- `rsrx_transport_adapter_send_application_data`:
  - outstanding send가 없으면 즉시 encode/send한다.
  - outstanding send가 있고 deferred slot이 비어 있으면 payload를 deferred slot에 복사하고 `OK`를 반환한다.
  - outstanding send와 deferred slot이 모두 차 있으면 `UNAVAILABLE`을 반환한다.
  ...
  - valid inbound message가 record되거나 correlated transport feedback이 수신되면 outstanding send를 clear하고, deferred slot이 있으면 즉시 다음 send를 dispatch한다.
- outbound telemetry:
