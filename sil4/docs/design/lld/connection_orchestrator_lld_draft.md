...
| `rsrx_action_executor_t` | struct | 단일 executor endpoint | context pointer와 callback을 함께 전달 |
| `rsrx_action_executor_table_t` | struct | transport/timer/api/diagnostics/lifecycle executor 집합 | 모든 executor 필수 |
| `rsrx_orchestrator_context_t` | struct | state machine context와 executor table 보유 | 동적 메모리 미사용 |
...
- 책임 분리:
  - orchestrator는 action category routing만 수행한다.
  - 실제 송신, 타이머 조작, 로그 기록은 executor 구현이 담당한다.
- executor 분류:
  - `transport`: handshake, inbound accept, heartbeat, retransmission request, disconnect, data delivery
  - `timer`: supervision timer start/reset
  - `api`: notify API
  - `diagnostics`: diagnostic logging
  - `lifecycle`: failsafe entry, resource release, retransmission context clear, shutdown finalize
- 오류 처리:
  - null context, null report, invalid executor table은 `RSRX_STATUS_INVALID_ARGUMENT`로 거부한다.
  - action에 대응하는 executor가 없으면 `RSRX_STATUS_INVALID_STATE`로 처리한다.
