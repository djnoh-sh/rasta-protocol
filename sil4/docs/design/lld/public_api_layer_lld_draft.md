...
| `rsrx_session_receive_transport_frame` | function | transport frame을 caller-owned output으로 수신 | initialized session과 유효 output buffer 필요 |
...
- `rsrx_session_record_inbound_message`:
  - critical-section 내부에서 accepted decoded inbound message를 session transport runtime에 기록한다.
  - transport supervisor inbound record path는 이 API를 사용해 session transport-adapter internals 직접 mutation을 줄인다.
  - invalid session/message 또는 critical-section enter/exit 실패 시 `INVALID_ARGUMENT`로 거부한다.
...
- `rsrx_session_receive_transport_frame`:
  - critical-section 내부에서 transport adapter receive를 수행하고 caller-owned output으로 frame을 반환한다.
  - transport supervisor poll receive path는 이 API를 사용해 session transport-adapter internals 직접 호출을 줄인다.
  - invalid session/output 또는 critical-section enter/exit 실패 시 output frame을 invalid/null/0/none baseline으로 clear하고 `INVALID_ARGUMENT`로 거부한다.
