...
코덱 계층의 상세 설계(LLD-008) 대비 실제 구현된 수신 가드 및 출력 버퍼 보호 로직의 검토 결과는 다음과 같습니다.

### 입력 포인터 NULL 검사 실패 시 조기 버퍼 클리어(Clear) 검증
*   **기술적 사실 및 근거:** `protocol_codec_lld_draft.md`에 정의된 "수신 디코딩 도중 입력 포인터 유효성 검사 실패 시 `INVALID_ARGUMENT`로 거부하며, 출력 버퍼가 유효할 경우 이를 청소해야 한다"는 설계 가이드라인에 부합하여, `rsrx_codec.c`의 `rsrx_codec_decode_frame`을 비롯한 모든 수신 디코딩 진입점에서 출력 구조체(`pxMessage`/`pxPacket`)가 NULL이 아닌지 우선 검사한 직후 `vClearDecodedMessage` 등을 먼저 호출하여 잔여 데이터를 완벽히 삭제하고 입력 포인터를 최종 검사하도록 안전 가드가 구현됨을 확인했습니다.

---

## 6. [V&V Backlog] 차기 마일스톤 진입 시 검증 필수 요구사항

*   **프로토콜 완전성 검증 단계:** RaSTA 규격 무결성 강화를 위한 `uRequireMac` 제어 시나리오 및 8바이트 타임스탬프 기반 시간 윈도우(Time Window) 검증 로직 구현 상태 확인(로드맵 `R-006` 과제 연계).
