...
### [V&V Finding F] 세션 리셋 시 타이머 명시적 정지(Stop) 처리 누락
*   **기술적 사실 및 근거:** `rsrx_api.c`의 `rsrx_session_reset` 함수가 실행 중이던 물리/가상 타이머들을 정지시키는 명시적인 플랫폼 API 호출 없이 세션 오케스트레이터와 채널 매니저만 리셋하는 한계가 확인되었습니다. 리셋 직후 유입되는 잔여 타이머 만료 인터럽트가 상태 기계의 예기치 않은 타임아웃 이벤트 주입으로 이어질 수 있으므로, 리셋 시 타이머의 명시적 비활성화 가드 추가를 권고합니다.
